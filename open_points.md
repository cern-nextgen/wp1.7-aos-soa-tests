...
# std::vector behavior
- If we have the templated feature in Oliver's current approach, to pass in std::vector / std::array / std::span, that is really nice. Then I'd e.g. like to be able to to emplace_back in the std::vector versions. $\color{blue}{I have an idea of how we could support this and other features of a specific container type.}$
